...
set_repository_binding() {
    ...
    local binding_json
    binding_json=$(printf '{"repository": "%s", "branch": "%s"}' "$repository" "$branch")
...
get_working_directory() {
    ...
    local repository branch repo_name
...
ensure_working_directory() {
    ...
    local repository branch repo_name work_dir
    repository=$(parse_binding "$binding" "repository")
    branch=$(parse_binding "$binding" "branch")
    ...
    repo_name=$(basename "$repository" .git)
    work_dir="$HYPE_REPOS_DIR/$hype_name/$repo_name/$branch"
    ...
    if [[ -d "$work_dir" ]]; then
        debug "Updating repository in $work_dir"
        (
            cd "$work_dir"
            git fetch origin
        ...
        debug "Cloning repository $repository to $work_dir"
