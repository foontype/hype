...
cmd_use() {
    ...
    if [[ $# -lt 2 ]] || [[ "$1" != "repo" ]]; then
        error "Usage: hype <hype-name> use repo <repository> [--branch <branch>]"
        exit 1
    fi
    ...
    set_repository_binding "$hype_name" "$repository" "$branch"
    ...
    local actual_branch
    actual_branch=$(parse_binding "$(get_repository_binding "$hype_name")" "branch")
...
cmd_list() {
    ...
    while IFS= read -r line; do
        ...
        if [[ -z "$binding_json" ]] || [[ "$binding_json" == "null" ]]; then
            ...
            local repository branch status
            repository=$(parse_binding "$binding_json" "repository")
            branch=$(parse_binding "$binding_json" "branch")
            ...
            if [[ ${#display_repo} -gt 40 ]]; then
                display_repo="${display_repo:0:37}..."
            fi
